...
with open(my_path + '/doc/random_search_output_regression.txt', 'a') as f:
    print('\n#############################################################', file=f)
    print('EXPECTED REJECTION - RANDOM SEARCH - PCA - RANDOM FOREST', file=f)
...
random_forest = RandomForestRegressor(n_estimators=565, max_depth=66, max_features=0.5257594207874666)
...
with open(my_path + '/doc/random_search_output_regression.txt', 'a') as f:
    print('\n#############################################################', file=f)
    print('INTERNALIZED STIGMA - RANDOM SEARCH - PCA - ELASTIC NET', file=f)
    print('\nBest Parameters', file=f)
...
# Specify the hyperparameters of the elastic net
elastic_net = ElasticNet(alpha=0.6329894722350858, l1_ratio=0.64708910017459, max_iter=561)
