...
# region HYPERPARAMETER TUNING - RANDOM SEARCH - ELASTIC NET - Prejudiced Events
...
model_en = random_search_en.fit(cmips_x, cmips_dheq)

# Save training results to file
with open(my_path + '/doc/random_search_output_regression.txt', 'a') as f:
    print('\n#############################################################', file=f)
    print('PREJUDICED EVENTS - RANDOM SEARCH - PCA - ELASTIC NET', file=f)
...
# Specify the hyperparameters of the elastic net
elastic_net = ElasticNet(alpha=0.6256413233388542, l1_ratio=0.6375378098331993, max_iter=566)
...
with open(my_path + '/doc/random_search_output_regression.txt', 'a') as f:
    print('\n#############################################################', file=f)
    print('IDENTITY CONCEALMENT - RANDOM SEARCH - PCA - RANDOM FOREST', file=f)
    print('\nBest Parameters', file=f)
...
random_forest = RandomForestRegressor(n_estimators=132, max_depth=79, max_features=0.6265744964288574)
